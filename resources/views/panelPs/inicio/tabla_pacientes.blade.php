...
@php
    ...
    $pacientesIds = psicologo_paciente::where('psicologo_id', $psicologo->id)->pluck('paciente_id');
  ...
  <table id="Tablapacientes" class="min-w-full border-collapse bg-white text-left text-sm text-gray-500">
    ...
    <tbody class="divide-y divide-gray-100 border-t border-gray-100">
      @foreach ($pacientes as $paciente)
        @php
          $userPaciente = User::find($paciente->id_user);
          $diagnostico = psicologo_paciente::where('paciente_id', $paciente->id)->where('psicologo_id', $psicologo->id)->first()->diagnostico;
        @endphp
        <tr class="hover:bg-gray-50">
          <th class="flex gap-3 px-6 py-4 font-normal text-gray-900 text-center items-center">
            <div class="relative h-10 w-10 flex-shrink-0">
              <img
                ...
                src="{{ asset('storage/' . $userPaciente->profile_photo_path) }}"
                alt="Foto de perfil"
              />
              <span class="absolute right-0 bottom-0 h-2 w-2 rounded-full bg-green-400 ring ring-white"></span>
            </div>
            <div class="text-sm sm:text-xs">
              <div class="font-medium text-gray-700">{{ $userPaciente->first_name }} {{ $userPaciente->last_name }}</div>
            </div>
          </th>
          <td class="px-6 py-4 text-center sm:px-4 sm:py-2">{{ $userPaciente->telefono }}</td>
          ...
          <td class="px-6 py-4 sm:px-4 sm:py-2">
            <div class="flex justify-center gap-4">
              ...
              <a x-data="{ tooltip: 'Ver' }" href="{{ route('verPacientes', ['id' => $paciente->id]) }}">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 24 24"
